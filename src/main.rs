...
fn main() {
    ...
    let day: u8 = match std::env::args().nth(1) {
        Some(s) => s.parse::<u8>().with_context(|| format!("Failed to parse first argument, got \"{}\", expected an integer in  [1, 25]", s)).expect("Parse error"),
        None => 1,
    };
    ...
    let part = match std::env::args().nth(2) {
        Some(part) => match part.as_str() {
            ...
            invalid_part_value => panic!("Invalid part specified, expected 1 or 2, got: {}", invalid_part_value),
        },
    ...
    assert!(
        day >= 1 && day <= 25,
        "Invalid day number; 1-25 is allowed"
    );

    // extract session ID from environment variable
    let session_id = std::env::var("AOC_SESSION").expect("Need a session key for looking up the input as environment variable AOC_SESSION!");

    let input = input_fetcher::fetch(day, session_id.as_str());
    let solution = exercises::solve_day(day, part, input);

    println!("Solution for exercise day {}, part {:?}:\n{}", day, part, solution);
}
