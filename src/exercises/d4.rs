...
fn parse_range(s: &str) -> RangeInclusive<i32> {
    ...
    let a = bounds.next().expect("missing lower bound").parse::<i32>().expect("parsing lower bound failed");
    let b = bounds.next().expect("missing upper bound").parse::<i32>().expect("parsing upper bound failed");
